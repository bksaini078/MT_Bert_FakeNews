...
def report_writing(args,fold,Model,lr,Batch_Size, Epoch,Alpha,Ratio, train_accuracy,test_accuracy,precision_true,precision_fake,recall_true,recall_fake,f1score_true,f1score_fake,AUC,comment):
    ...
    my_file = Path('Reports/'+args.data+'/' + args.model+'_'+args.method+'_'+str(fold)+'.csv')
    print(my_file)
    if my_file.exists():
