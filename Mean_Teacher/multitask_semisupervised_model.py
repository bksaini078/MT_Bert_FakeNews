...
# import Conv2D
# import Dense
from weight_norm_layers import Conv2D
from weight_norm_layers import Dense
...
def pi_model_gradients(X_train_labeled, y_train_labeled, X_train_unlabeled,pi_model, unsupervised_weight):
    ...
    """
    with tf.GradientTape() as tape:
        loss_value = pi_model_loss(X_train_labeled, y_train_labeled, X_train_unlabeled,pi_model, unsupervised_weight)
...
class PiModel(tf.keras.Model):
    ...
    def __init__(self):
        ...
        self._conv1a = Conv2D.Conv2D(filters=128, kernel_size=[3, 3],
                                              ...
                                              bias_initializer=tf.keras.initializers.constant(0.1),weight_norm=True, mean_only_batch_norm=True)
        self._conv1b = Conv2D.Conv2D(filters=128, kernel_size=[3, 3],
                                                        padding="same", activation=tf.keras.layers.LeakyReLU(alpha=0.1),
                                                        kernel_initializer=tf.keras.initializers.he_uniform(),
                                                        bias_initializer=tf.keras.initializers.constant(
                                                            0.1),weight_norm=True, mean_only_batch_norm=True)
    ...
    def __apply_image_augmentation(self, image):
        ...
        random_transformations = tfa.image.translations_to_projective_transforms(
            random_shifts)
        image = tfa.image.transform(image, random_transformations, 'NEAREST',
